...
let str_to_parser str =
  let fn item = match item.pstr_desc with
    | Pstr_value(_,ls) ->
       let gn vb =
         ...
         let lexify exp =
           ...
           | Pexp_constant (Pconst_char _) ->
              Exp.apply (grmod "term")
                [(Nolabel,Exp.apply (lxmod "char") [(Nolabel, exp);(Nolabel, unit_)])]
           | Pexp_constant (Pconst_string _) ->
              Exp.apply (grmod "term")
                [(Nolabel,Exp.apply (lxmod "string") [(Nolabel, exp);(Nolabel, unit_)])]
           | Pexp_construct({txt = Lident "INT"; _}, None) ->
              Exp.apply (grmod "term")
                [(Nolabel,Exp.apply (lxmod "int") [(Nolabel, unit_)])]
           | Pexp_construct({txt = Lident "FLOAT"; _}, None) ->
              Exp.apply (grmod "term")
                [(Nolabel,Exp.apply (lxmod "float") [(Nolabel, unit_)])]
           | _ -> exp
         in
         let item e =  match e.pexp_desc with
           | Pexp_construct({txt = Lident "::"; _},
                            Some({pexp_desc = Pexp_tuple
                                   ...
                                   ; exp ]; _})) ->
              (Pat.var {loc; txt = name},lexify exp)
           | _ ->
              (Pat.any (), lexify e)
         in
         ...
         let rec rules e = match e.pexp_desc with
           | Pexp_apply({ pexp_desc = Pexp_ident {txt = Lident "=>"; _}; _ },
              ...
              let fn exp name =
                Exp.fun_ Nolabel None name exp
              in
              let action =
                List.fold_left fn action args
              in
              let action =
                Exp.apply (grmod "empty") [(Nolabel,action)]
              in
              let fn item exp =
                Exp.apply (grmod "seqf") [(Nolabel,item);(Nolabel,exp)]
              in
              let rule =
                List.fold_right fn rule action
              in
              [rule]
           | Pexp_sequence(e1,e2) ->
              rules e1 @ rules e2
       ...
       let fail = Exp.apply (grmod "fail") [(Nolabel,unit_)] in
       let fn rule exp =
         Exp.apply (grmod "alt") [(Nolabel,rule);(Nolabel,exp)]
       in
       List.map
  ...
  let declarations =
    let gn (name, _) =
      Str.value Nonrecursive
        [Vb.mk (Pat.var name)
           (Exp.apply (grmod "declare_grammar")
              [(Nolabel, Exp.constant (Const.string name.txt))])]
    in
    List.map gn l
  in
  let definitions =
    let fn (name, rules) =
      Str.value Nonrecursive
        [Vb.mk (Pat.any ())
           (Exp.apply (grmod "set_grammar")
              [(Nolabel, Exp.ident (Location.mkloc (Lident name.txt) name.loc))
              ;(Nolabel, rules)])]
    in
    List.map fn l
  in
