...
let dseq g1 g2 = mkg (if g1.d = Fail then Fail else DSeq(g1,g2))
...
let factor_empty g =
  ...
  and kn : type a. a grdf -> a option = function
    ...
    | DSeq(g1,g2) -> fn g1; (match g1.e with
                       | None -> None
                       | Some (x,x') ->
                          try
                            let g2 = g2 x in
...
let first_charset : type a. a grne -> Charset.t = fun g ->
  ...
  let rec fn : type a. a grne -> bool * Charset.t = fun g ->
    ...
    | EDSeq(g,_) ->
       ...
       if shift then (false, Charset.full) else r (* FIXME *)
    | EAppl(g,_) -> fn g
    | ELr(g,_,_) -> fn g
...
let rec compile_ne : type a. a grne -> a Comb.t = fun g ->
  ...
  | ESeq(g1,g2) ->
     let cg = compile false g2 in
     Comb.seq (compile_ne g1) g2.e (first_charset g2.ne) cg
  | EDSeq(g1,g2) ->
     Comb.dseq (compile_ne g1)
       (memo (fun x -> let g2 = g2 x in
                 let cg = compile false g2 in
                 (g2.e, first_charset g2.ne, cg)))
