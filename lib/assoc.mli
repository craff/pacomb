...
(** [add  k v  l] inserts a  new binding  of [k] to  [v] at the  head of  [l]. A
   previous binding of [k] will not  be removed. Hence removing [k] will uncover
   a previous binding. *)
val add : 'a key -> 'a -> t -> t
...
(** Variation on the  above to associate value of type ['a data]  to key of type
   ['a key]. The  abobe function are obatained with [Make(struct  type 'a data =
   'a end)] *)
